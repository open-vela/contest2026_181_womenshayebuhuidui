...
`logs/pan_soak.py 60`，每 60 s 一轮（5×56B ping 公网 + 3×1472B ping 网关 + free），
盯 assert / 断链 / 丢包 / 堆用量。结果见 `logs/pan_soak.log` 与
`logs/pan_soak.out`，判定标准：

- `asserts = 0`
- 堆 `used` 不单调增长
- 小包 0% 丢包；大包丢包率不上升
- 断链后必须自动恢复（`state=dhcp_ok` 重新出现）

> 前 8 轮（约 8 分钟）：0 assert、0 断链、全部 0% 丢包、`used` 稳定在
> 1,100,304 字节。完整 60 分钟结论以 `pan_soak.out` 末行的汇总为准。
