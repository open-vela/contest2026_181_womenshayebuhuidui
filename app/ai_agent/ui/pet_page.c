...
static void on_pet_clicked(lv_event_t *e)
{
    ...
    if (bubble_label != NULL)
    {
        ...
        if (lv_obj_has_flag(bubble_label, LV_OBJ_FLAG_HIDDEN))
        {
            ...
            lv_label_set_text(bubble_label, "你好呀，我是小云 ☁️\n有什么可以帮到你的吗？");
        }
        else
        {
...
/**
 * Pet blink animation callback (toggle visibility)
 */
...
static void pet_start_idle_animation(void)
{
    ...
    lv_anim_set_reverse_delay(&pet_y_anim, 0);
...
lv_obj_t *pet_page_create(void)
{
    ...
    lv_obj_set_style_text_font(back_label, &lv_font_simsun_12_cjk, 0);
    ...
    lv_label_set_text(title, "☁️ 小云 - 你的桌面伙伴");
    ...
    /* Pet image placeholder (cloud emoji as placeholder) */

    pet_image = lv_label_create(pet_page_container);
    lv_label_set_text(pet_image, "☁️");
    lv_obj_set_style_text_font(pet_image, &lv_font_montserrat_48, 0);
    lv_obj_set_style_text_color(pet_image, lv_color_hex(0xffffff), 0);
    lv_obj_set_pos(pet_image, 135, 60); /* Centered horizontally */
    ...
    lv_label_set_text(bubble_label, "你好呀，我是小云 ☁️\n有什么可以帮到你的吗？");
    lv_obj_set_style_text_font(bubble_label, &lv_font_simsun_14_cjk, 0);
    ...
    lv_obj_set_style_text_font(status_label, &lv_font_simsun_12_cjk, 0);
