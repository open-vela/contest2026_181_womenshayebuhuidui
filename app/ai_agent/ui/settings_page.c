 ...
 * (lv_font_simsun_* are small subsets -> missing glyphs -> 乱码) */
...
lv_obj_t *settings_page_create(void)
{
    ...
    lv_label_set_text(back_label, "< 返回"); /* 不用 ←（U+2190 不在字体中，会乱码） */
    ...
    lv_label_set_text(title, "设置"); /* 不用 ⚙️（U+2699 不在字体中，会乱码） */
    ...
    /* Note（不用 ⚠️：U+26A0 不在 ui_font_sans_16 中，会乱码） */
